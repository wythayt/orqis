...
from orqis.compiler.capability_builder import enrich_graph_with_capabilities
...
def compile_graph(
    ...
    *,
    ...
    capability_notes, _capability_warnings = enrich_graph_with_capabilities(graph_or_factory, compiled_graph, lgir0)
    pass_trace = [
        PassTraceIR(
        ...
        PassTraceIR(
            pass_name="extract-capabilities",
            description="Load and normalize assets, skills, tools, subagents, executors, and approval policies.",
            highlights=[
                f"{len(lgir0.assets)} assets",
                f"{len(lgir0.tools)} tools",
                f"{len(lgir0.skills)} skills",
                f"{len(lgir0.subagents)} subagents",
            ]
            + capability_notes[:2],
        ),
    ]
    ...
    if runtime_trace:
        pass_trace.append(
            PassTraceIR(
                ...
                description="Capture a baseline Pregel execution trace from the real compiled graph to inform work analysis.",
                highlights=[
    ...
    analysis = build_analysis(compiled_graph, lgir0, lgir1, runtime_trace=runtime_trace)
    pass_trace.append(
        PassTraceIR(
            pass_name="static-analysis",
            description="Infer read/write sets, reducers, fanout, loops, side effects, cache safety, and runtime-informed work profiles.",
            highlights=[
                ...
                f"{len(analysis.work_profiles)} work profiles",
            ],
        )
    )
    ...
    pass_trace.append(
        PassTraceIR(
            pass_name="srv-plan",
            description=f"Lower partition IR to an AWS-oriented serverless deployment plan with `{resource_policy}` resource policy.",
            highlights=[
                ...
                f"slo_profile={srv_plan.compute.get('resource_summary', {}).get('slo_profile_id')}",
            ],
        )
    )

    next_steps = [
        "Emit deployable AWS IaC from srv_plan assets, tooling, agent_runtime, and compute sections.",
        "Replace heuristic capability manifests with decorators or typed registries for tools, skills, and subagents.",
        ...
        "Deepen lowering for nested subgraphs, provider-native tools, MCP services, and human approval resumes.",
    ]
